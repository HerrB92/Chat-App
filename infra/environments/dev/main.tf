...
# Must come after all resources that need RBAC assignments
module "identity" {
...
module "chat_api" {
  ...
  # Dev: minimal resources with scale-to-zero
  cpu          = 0.25
  memory       = "0.5Gi"
  min_replicas = 0
  max_replicas = 3
  ...
  environment_variables = [
    ...
    # Key Vault secret names (not the values themselves)
    ...
    # Managed Identity client ID — picked up automatically by DefaultAzureCredential
    ...
    # OpenAI endpoints — no API key; auth via Managed Identity
    ...
    { name = "CHAT_LOG_LEVEL", value = "info" },
...
# NEXT_PUBLIC_* variables are baked in at build time (Next.js limitation).
# A new image must be built when these values change.
module "chat_web" {
  source = "../../modules/container-app"
